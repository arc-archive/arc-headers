...
import '../headers-editor.js';
import { viewModel, autocompleteRef, resetCopyState } from '../src/internals.js';
...
accept-encoding: gzip`;
    ...
    it('switches editor from the switch button', async () => {
      ...
      assert.ok(cm, 'Code mirror element is in the dom');
    });
    ...
    it('editor has the value', async () => {
      ...
      await nextFrame();
      const cm = element.shadowRoot.querySelector('code-mirror');
      assert.equal(cm.editor.getValue(), headers);
    });
    ...
    it('updates the model and the value form the editor', async () => {
      ...
      await nextFrame();
      const cm = element.shadowRoot.querySelector('code-mirror');
      cm.editor.setValue('a: b');
      cm.dispatchEvent(new CustomEvent('input'))
